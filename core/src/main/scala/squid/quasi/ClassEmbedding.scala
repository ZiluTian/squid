    ...
    annottees match {
      ...
      if vparamss exists (_ exists (_.rhs nonEmpty)) =>
        ...
        rec(vparamss) map {
          ...
          case (vparamss2,rhs2) =>
            val mods2 = Modifiers(mods.flags, mods.privateWithin, 
              if (addSugarPhase) SUGAR_PHASE :: mods.annotations
              else mods.annotations)
              // ^ Note: we don't want to add the 'Sugar phase to overloaded methods generated by the overload encoding
              //   in case the class is not actually @embed'ed, as that would generate annoying warnings saying the
              //   method could not be inlined (desugared) whereas it was never actually intended to be...
